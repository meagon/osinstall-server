...
-- 创建同 mysql 一样的 unix_timestamp 函数
CREATE OR REPLACE FUNCTION unix_timestamp(input timestamp with time zone)
  RETURNS integer AS
$BODY$

BEGIN
		RETURN  EXTRACT(EPOCH FROM input)::int;
END;
   $BODY$
   LANGUAGE plpgsql;
...
CREATE TABLE  IF NOT EXISTS devices (
   ...
   "network_id"   int CHECK ("network_id" >= 0) NOT NULL,
   "os_id"        int CHECK ("os_id" >= 0) NOT NULL,
   "hardware_id"  int CHECK ("hardware_id" >= 0) DEFAULT NULL,
   "system_id"    int CHECK ("system_id" >= 0) NOT NULL,
   ...
   "install_log"        text, 
   "is_support_vm"      varchar CHECK ("is_support_vm" IN ( 'Yes','No' )) DEFAULT 'No' ,
   "user_id"            int NOT NULL DEFAULT '0', 
   "manage_ip"          varchar(32) DEFAULT NULL, 
   "manage_network_id"  int DEFAULT '0' 
);
...
CREATE TABLE  IF NOT EXISTS device_histories (
   ...
   "install_progress"   decimal(11,4) DEFAULT '0.0000', 
   "install_log"   text, 
   "is_support_vm" varchar CHECK ("is_support_vm" IN ( 'Yes','No' )) DEFAULT 'Yes' ,
   "user_id"   int NOT NULL DEFAULT '0', 
   "manage_ip"   varchar(32) DEFAULT NULL, 
   "manage_network_id"   int DEFAULT '0' 
);
...
DROP TABLE IF EXISTS device_install_callbacks CASCADE;
...
CREATE TABLE  IF NOT EXISTS hardwares (
   ...
   "oob"   text, 
   "bios"   text, 
   "is_system_add" varchar CHECK ("is_system_add" IN ( 'Yes','No' )) NOT NULL DEFAULT 'Yes' ,
   "tpl"   text, 
   "data"   text, 
   "source"   varchar(255) DEFAULT NULL, 
   "version"   varchar(255) DEFAULT NULL, 
   "status" varchar CHECK ("status" IN ( 'Pending','Success','Failure' )) DEFAULT 'Success'
);
...
CREATE TABLE  IF NOT EXISTS macs (
   ...
   "updated_at"   timestamp NULL DEFAULT NULL, 
   "deleted_at"   timestamp NULL DEFAULT NULL, 
   "device_id" int CHECK ("device_id" >= 0) NOT NULL,
...
DROP TABLE IF EXISTS manage_ips CASCADE;
...
DROP TABLE IF EXISTS manage_networks CASCADE;
...
CREATE TABLE  IF NOT EXISTS manufacturers (
   ...
   "memory"   text, 
   "disk"   text, 
   "cpu_sum"   int DEFAULT '0' , 
   "memory_sum"   int DEFAULT '0' , 
   "disk_sum"   int DEFAULT '0' , 
   "motherboard"   text, 
   "raid"   text, 
   "oob"   text, 
   "user_id"   int NOT NULL DEFAULT '0', 
   "is_vm" varchar CHECK ("is_vm" IN ( 'Yes','No' )) NOT NULL DEFAULT 'No',
   "is_show_in_scan_list" varchar CHECK ("is_show_in_scan_list" IN ( 'Yes','No' )) NOT NULL DEFAULT 'Yes',
   "nic_device"   text 
);
...
DROP TABLE IF EXISTS os_config CASCADE;
...
DROP TABLE IF EXISTS users CASCADE\g
CREATE TABLE  IF NOT EXISTS users (
   ...
   "permission"   text , 
 "status" varchar CHECK ("status" IN ( 'Enable','Disable' )) NOT NULL DEFAULT 'Enable',
 "role" varchar CHECK ("role" IN ( 'Administrator','User' )) DEFAULT 'User'
)   ;
...
DROP TABLE IF EXISTS vm_devices CASCADE;
CREATE TABLE  IF NOT EXISTS vm_devices (
   ...
   "os_id"   int NOT NULL, 
   "cpu_cores_number"   int NOT NULL, 
 "cpu_hot_plug" varchar CHECK ("cpu_hot_plug" IN ( 'Yes','No' )) NOT NULL DEFAULT 'No',
 "cpu_passthrough" varchar CHECK ("cpu_passthrough" IN ( 'Yes','No' )) NOT NULL DEFAULT 'No',
   "cpu_top_sockets"   int DEFAULT '0', 
   "cpu_top_cores"   int DEFAULT '0', 
   "cpu_top_threads"   int DEFAULT '0', 
   "cpu_pinning"   text, 
   "memory_current"   int DEFAULT '0', 
   "memory_max"   int DEFAULT '0', 
 "memory_ksm" varchar CHECK ("memory_ksm" IN ( 'Yes','No' )) NOT NULL DEFAULT 'No',
 ...
 "display_update_password" varchar CHECK ("display_update_password" IN ( 'Yes','No' )) NOT NULL DEFAULT 'No',
   "status"   varchar(255) NOT NULL, 
   "user_id"   int NOT NULL DEFAULT '0', 
   "system_id"   int NOT NULL DEFAULT '0' , 
   "install_progress"   decimal(11,4) NOT NULL DEFAULT '0.0000' , 
   "vnc_port"   varchar(256) DEFAULT NULL , 
   "run_status"   varchar(256) DEFAULT NULL  
)  ;
...
DROP TABLE IF EXISTS vm_device_logs CASCADE;
...
DROP TABLE IF EXISTS vm_hosts CASCADE;
CREATE TABLE  IF NOT EXISTS vm_hosts (
   ...
   "disk_available"   int DEFAULT '0' , 
   "vm_num"   int DEFAULT '0' , 
 "is_available" varchar CHECK ("is_available" IN ( 'Yes','No' )) DEFAULT 'Yes' ,
   "remark"   text  
);
...
-- ADD UNIQUE KEY sn (sn), ADD UNIQUE KEY ip (ip), ADD KEY batch_number (batch_number), ADD KEY status (status) USING BTREE, ADD KEY user_id (user_id), ADD KEY manage_ip (manage_ip);

ALTER TABLE device_histories ADD PRIMARY KEY (id);
--ADD KEY batch_number (batch_number), ADD KEY status (status) USING BTREE, ADD KEY sn (sn) USING BTREE, ADD KEY hostname (hostname) USING BTREE, ADD KEY user_id (user_id);
ALTER TABLE device_install_callbacks
  ADD PRIMARY KEY (id);

--ADD KEY device_id (device_id) USING BTREE;

ALTER TABLE device_install_reports ADD PRIMARY KEY (id) ;
--ADD KEY user_id (user_id);
ALTER TABLE device_logs ADD PRIMARY KEY (id);
--ADD KEY device_id (device_id) USING BTREE;
ALTER TABLE dhcp_subnets ADD PRIMARY KEY (id);
ALTER TABLE hardwares ADD PRIMARY KEY (id);
ALTER TABLE ips ADD PRIMARY KEY (id);
--ADD UNIQUE KEY network_id_ip (network_id,ip), ADD KEY network_id (network_id) USING BTREE, ADD KEY ip (ip) USING BTREE;
ALTER TABLE locations
  ADD PRIMARY KEY (id);
  --ADD KEY pid (pid) USING BTREE;
ALTER TABLE macs
  ADD PRIMARY KEY (id);
  -- ADD UNIQUE KEY mac (mac), ADD KEY device_id (device_id) USING BTREE;
ALTER TABLE manage_ips
  ADD PRIMARY KEY (id);
  -- ADD UNIQUE KEY network_id_ip (network_id,ip), ADD KEY network_id (network_id) USING BTREE, ADD KEY ip (ip) USING BTREE;
ALTER TABLE manage_networks ADD PRIMARY KEY (id);
  -- ADD UNIQUE KEY network (network);
ALTER TABLE manufacturers ADD PRIMARY KEY (id);
  -- ADD UNIQUE KEY sn (sn), ADD KEY device_id (device_id), ADD KEY user_id (user_id);
ALTER TABLE networks ADD PRIMARY KEY (id);
  -- ADD UNIQUE KEY network (network);
ALTER TABLE os_configs ADD PRIMARY KEY (id);
  -- ADD UNIQUE KEY name (name);
ALTER TABLE platform_configs ADD PRIMARY KEY (id);
  -- ADD UNIQUE KEY name (name);
ALTER TABLE system_configs ADD PRIMARY KEY (id);
  -- ADD UNIQUE KEY name (name);
ALTER TABLE users ADD PRIMARY KEY (id);
  -- ADD UNIQUE KEY username (username) USING BTREE;
ALTER TABLE user_access_tokens ADD PRIMARY KEY (id);
  -- ADD UNIQUE KEY access_token (access_token) USING BTREE;
ALTER TABLE vm_devices ADD PRIMARY KEY (id);
  -- ADD UNIQUE KEY hostname (hostname), ADD UNIQUE KEY ip (ip), ADD UNIQUE KEY mac (mac), ADD KEY status (status) USING BTREE, ADD KEY user_id (user_id);
ALTER TABLE vm_device_logs ADD PRIMARY KEY (id);
  -- ADD KEY device_id (device_id) USING BTREE;
ALTER TABLE vm_hosts ADD PRIMARY KEY (id);
  -- ADD UNIQUE KEY sn (sn);


CREATE SEQUENCE devices_id_seq  START WITH 1  INCREMENT BY 1  NO MINVALUE  NO MAXVALUE  CACHE 1;  
ALTER TABLE devices alter column id set default nextval('devices_id_seq'); 

CREATE SEQUENCE device_histories_id_seq START WITH 1  INCREMENT BY 1  NO MINVALUE  NO MAXVALUE  CACHE 1;  
ALTER TABLE device_histories ALTER COLUMN id set default nextval('device_histories_id_seq');

CREATE SEQUENCE device_install_callbacks_id_seq START WITH 1 INCREMENT BY 1 NO MINVALUE  NO MAXVALUE  CACHE 1;  
ALTER TABLE device_install_callbacks ALTER COLUMN id set default nextval('device_install_callbacks_id_seq');

CREATE SEQUENCE device_install_reports_id_seq START WITH 1  INCREMENT BY 1  NO MINVALUE  NO MAXVALUE  CACHE 1;  
ALTER TABLE device_install_reports ALTER COLUMN id set default nextval('device_install_reports_id_seq');

CREATE SEQUENCE devices_logs_id_seq  START WITH 1  INCREMENT BY 1  NO MINVALUE  NO MAXVALUE  CACHE 1; 
ALTER TABLE device_logs ALTER COLUMN id set default nextval('devices_logs_id_seq') ;

CREATE SEQUENCE dhcp_subnets_id_seq  START WITH 1  INCREMENT BY 1  NO MINVALUE  NO MAXVALUE  CACHE 1; 
ALTER TABLE dhcp_subnets ALTER COLUMN id set default nextval('dhcp_subnets_id_seq') ;

CREATE SEQUENCE hardwares_id_seq  START WITH 1  INCREMENT BY 7 NO MINVALUE  NO MAXVALUE  CACHE 1;  
ALTER TABLE hardwares ALTER COLUMN id set default nextval('hardwares_id_seq');

CREATE SEQUENCE ips_id_seq  START WITH 1  INCREMENT BY 1  NO MINVALUE  NO MAXVALUE  CACHE 1;  
ALTER TABLE ips ALTER COLUMN id set default nextval('ips_id_seq');
...
CREATE SEQUENCE macs_id_seq  START WITH 1  INCREMENT BY 1  NO MINVALUE  NO MAXVALUE  CACHE 1;  
ALTER TABLE macs ALTER COLUMN id set default nextval('macs_id_seq');

CREATE SEQUENCE manage_ips_id_seq START WITH 1  INCREMENT BY 1  NO MINVALUE  NO MAXVALUE  CACHE 1;  
ALTER TABLE manage_ips  ALTER COLUMN id set default nextval('manage_ips_id_seq') ;

CREATE SEQUENCE manage_networks_id_seq  START WITH 1  INCREMENT BY 1  NO MINVALUE  NO MAXVALUE  CACHE 1;  
ALTER TABLE manage_networks ALTER COLUMN id set default nextval('manage_networks_id_seq');

CREATE SEQUENCE manufacturers_id_seq  START WITH 1  INCREMENT BY 1  NO MINVALUE  NO MAXVALUE  CACHE 1;  
ALTER TABLE manufacturers ALTER COLUMN id set default nextval('manufacturers_id_seq') ;

CREATE SEQUENCE networks_id_seq  START WITH 1  INCREMENT BY 1  NO MINVALUE  NO MAXVALUE  CACHE 1;  
ALTER TABLE networks ALTER COLUMN id set default nextval('networks_id_seq');

CREATE SEQUENCE os_configs_id_seq  START WITH 1  INCREMENT BY 9 NO MINVALUE  NO MAXVALUE  CACHE 1;  
ALTER TABLE os_configs ALTER COLUMN id set default nextval('os_configs_id_seq');

CREATE SEQUENCE platform_configs_id_seq  START WITH 1  INCREMENT BY 3 NO MINVALUE  NO MAXVALUE  CACHE 1;  
ALTER TABLE platform_configs ALTER COLUMN id set default nextval('platform_configs_id_seq');


CREATE SEQUENCE system_configs_id_seq  START WITH 1  INCREMENT BY 13 NO MINVALUE  NO MAXVALUE  CACHE 1;  
ALTER TABLE system_configs ALTER COLUMN id SET DEFAULT nextval('system_configs_id_seq');


CREATE SEQUENCE users_id_seq  START WITH 1  INCREMENT BY 3 NO MINVALUE  NO MAXVALUE  CACHE 1;  
ALTER TABLE users ALTER COLUMN id SET DEFAULT nextval('users_id_seq');

CREATE SEQUENCE user_access_tokens_id_seq START WITH 1  INCREMENT BY 1  NO MINVALUE  NO MAXVALUE  CACHE 1;  
ALTER TABLE user_access_tokens ALTER COLUMN id set default nextval('user_access_tokens_id_seq');

CREATE SEQUENCE vm_devices_id_seq  START WITH 1  INCREMENT BY 1  NO MINVALUE  NO MAXVALUE  CACHE 1;  
ALTER TABLE vm_devices ALTER COLUMN id set default nextval('vm_devices_id_seq');

CREATE SEQUENCE vm_device_logs_id_seq  START WITH 1  INCREMENT BY 1  NO MINVALUE  NO MAXVALUE  CACHE 1;  
ALTER TABLE vm_device_logs ALTER COLUMN id set default nextval('vm_device_logs_id_seq');

CREATE SEQUENCE vm_hosts_id_seq  START WITH 1  INCREMENT BY 1  NO MINVALUE  NO MAXVALUE  CACHE 1;  
ALTER TABLE vm_hosts alter column id set default nextval('vm_hosts_id_seq');



COMMENT ON COLUMN devices."is_support_vm" IS '是否支持安装虚拟机';
COMMENT ON COLUMN device_histories."is_support_vm" IS '是否支持安装虚拟机';
COMMENT ON COLUMN hardwares."is_system_add" IS '是否是系统添加的配置';
COMMENT ON COLUMN manufacturers.sn IS '序列号';
COMMENT ON COLUMN manufacturers.mac IS 'mac地址';
COMMENT ON COLUMN manufacturers.cpu_sum IS 'CPU总核数';
COMMENT ON COLUMN manufacturers.memory_sum IS '内存总容量';
COMMENT ON COLUMN manufacturers.disk_sum IS '硬盘总容量';
COMMENT ON COLUMN users.phone_number IS '手机号';
COMMENT ON COLUMN users.permission IS '权限';
COMMENT ON COLUMN vm_devices.system_id IS '系统安装模板ID';
COMMENT ON COLUMN vm_devices.install_progress IS '安装进度';
COMMENT ON COLUMN vm_devices.vnc_port IS 'VNC端口';
COMMENT ON COLUMN vm_devices.run_status IS '运行状态';
COMMENT ON COLUMN vm_hosts.sn IS '序列号';
COMMENT ON COLUMN vm_hosts.cpu_sum IS 'CPU总核数';
COMMENT ON COLUMN vm_hosts.cpu_used IS '已用CPU核数';
COMMENT ON COLUMN vm_hosts.cpu_available IS '可用CPU核数';
COMMENT ON COLUMN vm_hosts.memory_sum IS '内存总容量';
COMMENT ON COLUMN vm_hosts.memory_used IS '已用内存';
COMMENT ON COLUMN vm_hosts.memory_available IS '可用内存';
COMMENT ON COLUMN vm_hosts.disk_sum  IS '磁盘总容量';
COMMENT ON COLUMN vm_hosts.disk_used IS '已用磁盘空间';
COMMENT ON COLUMN vm_hosts.disk_available IS '可用磁盘空间';
COMMENT ON COLUMN vm_hosts.vm_num IS '虚拟机数量';
COMMENT ON COLUMN vm_hosts.is_available IS '是否可用';
COMMENT ON COLUMN vm_hosts.remark IS '备注(不可用的原因，等等)';
